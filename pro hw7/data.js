...
const getAvaregeSum = (acc, curr) => acc + curr;
const getStudentsAvarege = item => {
  return {
    name: item.name,
    specialty: item.specialty,
    marks: item.marks,
    average: Math.round(item.marks.reduce(getAvaregeSum) / item.marks.length)
  }
};

const result = students.map(getStudentsAvarege);
console.log(result);




const getWorstStudents = (item) => {
  const minScore = 60;
  if (item.average < minScore) {
    return item.name
  }
};

const WorstStudents = result.filter(getWorstStudents);
...
const getStudentsMeridian = item => {
  return {
    name: item.name,
    average: Math.round(item.marks.reduce(getAvaregeSum) / item.marks.length)
  }
};
const meridian = students.map(getStudentsMeridian);
...
console.log(addStudent(students, 'Frosya Shehirizadova', 'Flute Playing',100));




const getSortStudent = meridian.sort((x, y) =>
y.average - x.average);
console.log(getSortStudent);




function getLargestAverage(numbers) {
  return numbers.slice().sort((a, b) => b - a).slice(0, 5);
}

console.log(getLargestNumbers(meridian))
